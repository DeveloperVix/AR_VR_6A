...
public class PoolingManager : MonoBehaviour
{
    ...
    {
        for (int i = 0; i < TamAlberca; i++)
        {
            GameObject Prefab = Instantiate(PreAlberca);
            AlbercaFin.Enqueue(PreAlberca);
            PreAlberca.SetActive(false);
        }
    }
    ...
    public GameObject Shoot(Vector3 newpos, Quaternion newrot)
    {
        GameObject Activar;
        Activar = AlbercaFin.Dequeue();
        Activar.SetActive(true);
        Activar.transform.position = newpos;
        Activar.transform.rotation = newrot;
        return Activar;
    }
