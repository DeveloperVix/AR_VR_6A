...
public class GameControlerUI : MonoBehaviour
{
    ...
    private void OnEnable()//si es verdadero est emetodo se manda a llamar
    {
        ...
        SceneManager.sceneLoaded += OnSceneLoaded;//le estamos añadiendo nuestro metodo propio
    }

    void OnDisable()
    {
        Debug.LogError("OnDisable llamado");
        SceneManager.sceneLoaded -= OnSceneLoaded;//le estamos quitando nuestro metodo propio
    }
